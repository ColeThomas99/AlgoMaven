...
using AlgoMaven.Core.MarketData.Binance;
using AlgoMaven.Core.MarketData.Binance.Requests;
using AlgoMaven.Core.MarketData.Binance.Responses;
...
namespace AlgoMaven.Core.MarketData
{
	public class BinanceMarketDataAPI : MarketDataAPIBase
	{
        ...
        private string CraftPriceRequest(BinanceMarketPriceRequest request)
        {
            ...
            args.Add("symbol", $"{request.Symbol}USDT");
            args.Add("startTime", request.StartTime?.ToString());
            args.Add("endTime", request.EndTime?.ToString());
            args.Add("interval", interval);
            args.Add("limit", request.Limit?.ToString());

            foreach (KeyValuePair<string, string> arg in args)
                endPoint += $"{arg.Key}={arg.Value}&";
